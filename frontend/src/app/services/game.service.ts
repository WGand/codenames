...
export class GameService {
    ...
    private onSendHintError(error: any) {
        if (error instanceof HttpErrorResponse && error.status === 400) {
            this.snackBar.open(
                "¡Has ingresado algo incorrecto, colega! \nVerifica: se necesita EXACTAMENTE UNA PALABRA seguida de un ESPACIO y luego un NÚMERO del 0 al 9",
                    "Gracias", {
                    duration: 6000,
    ...
    private onUncoverAgentError(error: any) {
        if (error instanceof HttpErrorResponse && error.status === 400) {
            this.snackBar.open(
                "No es el momento de revelar, ¡estamos esperando el cifrado del Centro!",
                "De acuerdo, así sea", {
                    duration: 5000
                });
        }
    ...
    private onGameFinished() {
        this.snackBar.open(
            "El juego ha terminado. \nPresiona 'Crear nuevo juego' para continuar en la misma compañía.",
            "¡Genial!");
    }
